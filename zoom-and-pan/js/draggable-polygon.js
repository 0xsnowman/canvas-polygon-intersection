class DraggablePolygon {
  ...
  const { x, y } = this.getMousePosition(event);
    ...
    if (is_index_valid(dragPointIndex)) {
      ...
      this.points.forEach((point) =>
        this.originalPoints.push(new Point(point.x, point.y, point.isReal))
      );
  ...
  onMouseUp() {
    if (is_index_valid(this.draggingPointIndex)) {
      const angle = globalCameras.find(
        (cam) => cam.cameraID == this.cameraID
      ).m_angle;
      const out_radius = visionRanges(this.type, angle)[2] * this.scale;

      if (
        distance(this.points[this.draggingPointIndex], this.center) > out_radius
      ) {
        this.draggingPointIndex = null;
        this.points = [];
  ...
  restrictToRadius(centerX, centerY, mouseX, mouseY, radius, upOrDown) {
    const angle = toRadians(
      globalCameras.find((cam) => cam.cameraID == this.cameraID).m_angle
    );
  ...
  circularCutRestAndAddIntersect(
    arr,
    firstCutOutIndex,
    secondCutOutIndex,
    newPoint,
    directionDueToCamType
  ) {
    if (!Array.isArray(arr) || arr.length === 0) {
      throw new Error("Invalid array");
    }
  ...
  intersectAreaNotVisibleExist() {
    ...
    for (let i = 0; i < this.points.length; ++i) {
      if (
        i != this.draggingPointIndex &&
        (i + 1) % this.points.length != this.draggingPointIndex
      ) {
        const edge2Distanve = distance(
          this.center,
        ...
        const edge2 = [
          this.center,
          {
            x:
              this.center.x +
              (((this.points[this.draggingPointIndex].x - this.center.x) *
                out_radius) /
                edge2Distanve) *
                2,
            y:
              this.center.y +
              (((this.points[this.draggingPointIndex].y - this.center.y) *
                out_radius) /
                edge2Distanve) *
                2,
          },
  ...
  addPointsAroundDraggingPoint(arrivedPoint) {
    const arrivedPointIndex = this.points.findIndex((point) =>
      this.isPointClicked(point, arrivedPoint.x, arrivedPoint.y)
    );
